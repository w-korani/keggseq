...
kegg_map_enzymes <- function (species,dataset,deg_list,map_number,correction_file_name,deg)
{
	...
	Table3b = cbind(kegg_enzymes = as.character(Table2b[,1]),enzyme_discription = as.character(Table2b[,2]))
	if (deg == TRUE)
	{return (merge(Table6,Table3b,by="kegg_enzymes"))}
	else if (deg == FALSE)
	{return (merge(Table5,Table3b,by="kegg_enzymes"))}
	write.table(Table5,paste0(a1,"_enzymes.csv"),row.names = FASLE,sep=",")
